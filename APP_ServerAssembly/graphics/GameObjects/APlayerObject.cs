...
namespace Avril.ServerAssembly.Graphics.GameObjects
{
    public abstract class APlayerObject
    {
        ...
        private static int playerObjectCounter;
        public readonly int playerObjectNumber;
        ...
        public APlayerObject(ARenderable model)
        {
            _model = model;
            ...
            _position = new Vector3(0, 0, 1).Normalized() * 100f;
            ...
            float pitch = Vector3.CalculateAngle(new Vector3(_fowards.X, 0, 0), Get_axisX());
            float yaw = Vector3.CalculateAngle(new Vector3(0, _fowards.Y, 0), Get_axisY());
            float roll = Vector3.CalculateAngle(new Vector3(0, 0, _fowards.Z), Get_axisZ());
            _rotation = new Vector3(pitch, yaw, roll);
            Set_Rotation(Trim_Rotation_To_Fundermental_Octive(Get_Rotation()));
            System.Console.WriteLine("TESTBENCH => angle of first position: x = " + _rotation.X + "  y = " + _rotation.Y + "  z = " + _rotation.Z);

            _speed = 1.5f;
            _scale = new Vector3(1);
            playerObjectNumber = playerObjectCounter++;
        }
